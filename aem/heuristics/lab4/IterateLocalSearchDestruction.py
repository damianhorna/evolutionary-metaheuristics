...
class ItereteLocalSearchDestruction(SteepestEdgeSwapListOfMoves, Greedy):
    ...
    def run_experiment(self, cycle, time_limit=1):
        ...
        #    self.reset()
            ...
            # Destruction
            delete_n = int(len(cycle)/5) # 20%
            roll = np.random.randint(len(cycle))
            cycle = (cycle[roll:]+cycle[:roll])[:-delete_n]
            self.extend_cycle(cycle)
            self.reset()
